...
class RaysFromLight(Rays):
    def __init__(self, c2w, W, H, focal, near, far, device='cpu'):
        ...
        self.light_pos = c2w[:,-1].to(device)
        ...
        self.near, self.far, self.W, self.H, self.focal = near, far, W, H, focal # store everything
    ...
    def update_depth_map(self, batch_size, num_samples, L_x, L_dir, model):
        for i, rays_batch in enumerate(self.batchify(batch_size, random=False, crop=False)):
            ...
            depth = rays_batch.properties['depth']
